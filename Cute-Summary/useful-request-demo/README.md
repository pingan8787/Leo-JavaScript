...
分析上面问题后，我初步的方案如下：
参考**插件化架构设计**，**独立每个拦截器**，将每个拦截器抽离成单独文件维护，做到**职责单一**，然后通过**拦截器调度器**进行调度和执行。

其拦截器调度过程如下图：
![拦截器调度过程](https://images.pingan8787.com/image/http-interceptors/run.png)
...
这就是我们的**核心拦截器调度器**，目前实现导入所有请求拦截器和响应拦截器后，通过 `for` 循环，执行所有拦截器，最后将整个 axios 实例返回出去。
...
然后在导出该请求拦截器，并且导出的是个**数组**，方便拦截器调度器进行统一执行：
